...
class ArithmeticsTests: XCTestCase {
    ...
    // Mark: UInt8
    func testUInt8RotateLeft() {
        let val1 = 0x01 as UInt8
        XCTAssertEqual(val1.rotateLeft(0x01), 0x02, "Rotation test failed")
        XCTAssertEqual(val1.rotateLeft(0x02), 0x04, "Rotation test failed")

        let val2 = 0x05 as UInt8
        XCTAssertEqual(val2.rotateLeft(0x01), 0x0a, "Rotation test failed")
        XCTAssertEqual(val2.rotateLeft(0x02), 0x14, "Rotation test failed")
    }

    func testUInt8RotateRight() {
        let val1 = 0x01 as UInt8
        XCTAssertEqual(val1.rotateRight(0x01), 0x80, "Rotation test failed")
        XCTAssertEqual(val1.rotateRight(0x02), 0x40, "Rotation test failed")

        let val2 = 0x05 as UInt8
        XCTAssertEqual(val2.rotateRight(0x01), 0x82, "Rotation test failed")
        XCTAssertEqual(val2.rotateRight(0x02), 0x41, "Rotation test failed")
    }
